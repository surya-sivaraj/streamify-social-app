...
export async function getRecommendedUsers(req, res) {
  ...
  $and: [
    ...
    { isOnboarded: true },                            // Only users who finished onboarding
  ]
})
...
export async function acceptFriendRequest(req, res) {
    try {
        ...
        res.status(200).json({ message: "friendRequest accepted" });
...
export async function getFriendRequests(req,res){
    try {
        ...
        console.log("erroe in getpendingFriendsRequests controller",error.message);
        res.status(500).json({message:"internal server error"})
...
export async function getOutgoingFriendsReqs(req,res) {
 try {
    const outgoingRequests = await FriendRequest.find({
      ...
      console.log("error in outgoingFriendsRequests controller",error.message);
        res.status(500).json({message:"internal server error"})
